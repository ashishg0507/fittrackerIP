...
const CACHE_SCOPE_KEY = 'fitTracker_cache_scope';
...
function getCacheScope() {
    try {
        return localStorage.getItem(CACHE_SCOPE_KEY) || 'anon';
    } catch (_) {
        return 'anon';
    }
}

function setCacheScope(scope, options = {}) {
    try {
        const nextScope = (scope || 'anon').toString().trim() || 'anon';
        const currentScope = getCacheScope();
        const shouldClear = options.clearExisting !== false && currentScope !== nextScope;
        if (shouldClear) {
            clearAllCaches();
        }
        localStorage.setItem(CACHE_SCOPE_KEY, nextScope);
    } catch (err) {
        console.warn('Set cache scope error:', err);
    }
}

function getScopedCacheKey(key) {
    return `${CACHE_PREFIX}${getCacheScope()}_${key}`;
}
...
function getCache(key) {
    try {
        const cacheKey = getScopedCacheKey(key);
...
function setCache(key, data, ttl = null) {
    try {
        ...
        if (err.name === 'QuotaExceededError') {
            ...
            // Try once more
            try {
                const retryKey = getScopedCacheKey(key);
                ...
                localStorage.setItem(retryKey, JSON.stringify(cacheData));
            } catch (e) {
...
function clearCachePattern(pattern) {
    try {
        const scopedPrefix = `${CACHE_PREFIX}${getCacheScope()}_`;
        const keys = Object.keys(localStorage);
        keys.forEach(key => {
            if (key.startsWith(scopedPrefix) && key.includes(pattern)) {
...
if (typeof window !== 'undefined') {
    // Keep anonymous scope by default until sign-in sets it.
    if (!localStorage.getItem(CACHE_SCOPE_KEY)) {
        localStorage.setItem(CACHE_SCOPE_KEY, 'anon');
    }
